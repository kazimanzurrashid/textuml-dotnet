define (require) ->
  ...
  describe 'context', ->
    ...
    describe '#isCurrentDocumentDirty', ->
      ...
      describe 'new', ->
        ...
        before ->
          ...
          stubbedIsCurrentDocumentNew = sinon.stub context,
            'isCurrentDocumentNew', -> true

        describe 'with content', ->
          before ->
            context.setCurrentDocumentContent 'A -> B: Method'

          it 'returns true', ->
            expect(context.isCurrentDocumentDirty()).to.be.ok

        describe 'no content', ->
          before ->
            context.setCurrentDocumentContent ''

          it 'returns false', ->
            expect(context.isCurrentDocumentDirty()).to.not.be.ok

        after -> stubbedIsCurrentDocumentNew.restore()
      ...
      describe 'existing', ->
        ...
        describe 'same content', ->
          ...
          before ->
            stubbedDocumentGet = sinon.stub document,
              'get',
              -> context.getCurrentDocumentContent()

          it 'returns false', ->
            expect(context.isCurrentDocumentDirty()).to.not.be.ok

          after -> stubbedDocumentGet.restore()
